...
// Hotbar slot change → item name notification (replays vanilla behavior)
world.afterEvents.playerHotbarSelectedSlotChange.subscribe((event) => {
  const player = Server.instance.player(event.player.name);
  if (player) {
    player.changedHotbarSlot();
  }
  if (player && event.itemStack) {
    const name = event.itemStack.nameTag || event.itemStack.typeId.replace("minecraft:", "");
    player.showNotification(name, 2);
  }
});
...
world.afterEvents.worldLoad.subscribe(() => {
  ...
  for (const gamertag of allowlist) {
    world.getDimension("overworld").runCommand(`allowlist add "${gamertag.name}"`);
  }
});
