...
import { DamageSources } from './adapters/damage_sources.js'
import { ExplosionSources } from './adapters/explosion_sources.js'
...
// Peaceful damage shield
world.beforeEvents.entityHurt.subscribe((event) => {
  if (event.hurtEntity.typeId !== 'minecraft:player') return
  const source = DamageSources.resolve(event.damageSource)
  const player = Server.instance.player(event.hurtEntity.nameTag)
  if (player?.shouldShieldFrom(source)) event.cancel = true
})
// Explosion block protection
world.beforeEvents.explosion.subscribe((event) => {
  const source = ExplosionSources.resolve(event.source)
  const allowed = source.blocksAllowedToImpact(event.getImpactedBlocks())
  if (allowed !== null) event.setImpactedBlocks(allowed)
})
...
world.afterEvents.worldLoad.subscribe(() => {
  ...
  // Defer to next tick — runCommand needs the world fully ready
  system.run(() => {
