...
export class Player {
  ...
  constructor(mcPlayer) {
    ...
    if (this.#preferences.coords) this.#actionBar.enableCoords()
  }

  get name() { return this.#mcPlayer.name }
  get preferences() { return this.#preferences }
  get stats() { return this.#stats }
  get isOp() { return this.#mcPlayer.isOp() }
  get abstains() { return this.#presence.isAfk }
  get isAfk() { return this.#presence.isAfk }
  sendMessage(text) { this.#mcPlayer.sendMessage(text) }
  ...
  set home(location) { this.#preferences.home = location }

  goHome() {
    const home = this.#preferences.homeLocation
    if (home) this.#mcPlayer.teleport({ x: home.x, y: home.y, z: home.z })
    return this.location
  }

  toggleCoords() {
    const newValue = this.#preferences.toggleCoords()
    if (newValue) this.#actionBar.enableCoords()
    else this.#actionBar.disableCoords()
    return newValue
  }

  showNotification(text, seconds) { this.#actionBar.notify(text, seconds) }
  updateActionBar() { this.#actionBar.tick() }
  ...
  gazedAt(block) { block.gazedAtBy(this) }
  ...
  shouldShieldFrom(damageSource) { return this.#preferences.peaceful && damageSource.isHostile }
  changedHotbarSlot() { this.#activity.touch() }
  ...
  played(seconds) {
    ...
    if (!this.#presence.isAfk) this.#stats.playTime.increment(seconds)
    const distance = this.#activity.distanceMoved
    if (distance > 0) this.#stats.distanceWalked.increment(distance)
  }
}
