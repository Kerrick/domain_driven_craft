...
export class DifficultyPoll {
  ...
  markWarned() {
    ...
    this.#server.speak(msg => {
      msg.line(`${msg.warning('[Difficulty]')} ${this.#deadline.remainingSeconds}s left to vote!`);
    });
  }
  
  announceExpiration() {
    this.#server.speak(msg => {
      msg.line(`${msg.error('[Difficulty]')} Vote for ${msg.highlight(this.#targetDifficulty)} timed out`);
    });
  }
  
  vote(player) {
    if (this.isExpired) {
      this.#server.whisper(player, msg => msg.line(msg.error('This vote has expired')));
      return;
    }
  ...
  #announce(voter) {
    ...
    this.#server.speak(msg => {
      msg.line(`${msg.success('[Difficulty]')} ${msg.highlight(voter.name)} voted for ${msg.highlight(this.#targetDifficulty)}! ${msg.muted(`(${current}/${needed})`)}`);
      msg.line(`${msg.muted('Type')} ${msg.command(`!difficulty ${this.#targetDifficulty}`)} ${msg.muted('to vote.')} ${msg.muted(`${this.#deadline.remainingSeconds}s remaining`)}`);
    });
  }
  ...
  #resolve() {
    ...
    this.#server.speak(msg => {
      msg.line(`${msg.success(msg.bold('[Difficulty]'))} Vote passed! Difficulty: ${msg.highlight(this.#targetDifficulty)} for ${this.#effect.durationSeconds}s`);
    });
  ...
  #effectExpired() {
    this.#server.speak(msg => msg.line(`${msg.warning('[Difficulty]')} Override expired, recalculating...`));
  ...
  conditionChanged() {
    ...
    if (needsPeaceful && this.#targetDifficulty !== "peaceful") {
      this.#server.speak(msg => {
        msg.line(`${msg.warning('[Difficulty]')} Peaceful player joined, vote cancelled`);
      });
      this.#server.clearPoll(this);
      this.#server.recalculateDifficulty();
    }
  ...
  playerLeft(name) {
    ...
    if (this.#ballot.count > 0) {
      this.#server.speak(msg => msg.line(`${msg.warning('[Difficulty]')} ${name} left. Vote continues.`));
