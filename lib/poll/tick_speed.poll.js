...
import { muted, highlight, success, warning, command } from '../server/styles.js'
...
export class TickSpeedPoll extends Poll {
  ...
  announceExpiration() { Chat.instance.speak(`${muted`${this.tag}`} Vote timed out — not enough support`) } // eslint-disable-line sonarjs/no-nested-template-literals

  announceVote(voter) {
    const needed = Server.instance.activePlayers().size
    Chat.instance.speak(
  ...
  resolve() {
    Settings.instance.apply('tickSpeed', Change.temporary(this.#speed, this.#duration))
    Chat.instance.speak([success`${this.tag}`, 'Vote passed! Tick speed:', highlight`${this.#speed}`, `for ${this.#duration}s`])
  }

  announcePlayerLeft(name) { Chat.instance.speak([warning`${this.tag}`, `${name} left. Vote continues.`]) }
}
