...
export class TickSpeedPoll extends Poll {
  ...
  resolve() {
    ...
    Chat.instance.speak([success`${this.tag}`, `Vote passed! Tick speed:`, highlight`${this.#speed}`, `for ${this.effectDuration}s`]);
  }
  
  effectExpired() {
    Server.instance.tickSpeed = 1;
    Chat.instance.speak([warning`${this.tag}`, `Tick speed returned to normal`]);
  }
