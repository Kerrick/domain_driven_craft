...
export class Poll {
  ...
  constructor() { this.#deadline = new Deadline(Settings.instance.pollTimeout.current) }
  ...
  markWarned() {
    ...
    Chat.instance.speak(`${warning`${this.tag}`} ${this.remainingSeconds}s left to vote!`) // eslint-disable-line sonarjs/no-nested-template-literals
  }
  ...
  hasVote(name) { return this.#ballot.has(name) }
