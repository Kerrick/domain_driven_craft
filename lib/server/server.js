...
import { highlight } from './styles.js'
...
export class Server extends Singleton {
  ...
  get settings() { return Settings.instance }
  ...
  tick() {
    ...
    if (elapsedSeconds > 0) {
      ...
      for (const player of this.#players.values()) player.played(elapsedSeconds)
    }
  ...
  player(name) { return this.#players.get(name) }
  allPlayers() { return new Set(this.#players.values()) }
  activePlayers() { return new Set([...this.allPlayers()].filter((p) => !p.isAfk)) }
  ...
  playerLeft(name) {
    ...
    for (const poll of Chat.instance.activePolls()) poll.playerLeft(name)
    this.recalculateDifficulty()
  }
