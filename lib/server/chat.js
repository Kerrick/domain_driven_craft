...
export class Chat extends Singleton {
  ...
  whisper(player, ...lines) {
    for (const line of lines) player.sendMessage(formatLine(line))
  }

  speak(...lines) {
    for (const line of lines) world.sendMessage(formatLine(line))
  }

  broadcastWarning(...messageParts) { this.speak([warning`[Server]`, ...messageParts]) }
  broadcastSuccess(...messageParts) { this.speak([success`[Server]`, ...messageParts]) }

  celebrate(tag, ...messageParts) {
    this.speak([success`${bold`★`}`, highlight`${tag}`, ...messageParts, success`${bold`★`}`]) // eslint-disable-line sonarjs/no-nested-template-literals
  }
  ...
  get pollSummary() { return this.#polls.asChatSummary }
