...
export class Welcome {
  ...
  greet(player) {
    system.runTimeout(() => {
      ...
      const lines = [
        success`${bold`★ Welcome to ${this.#server.name}! ★`}`, // eslint-disable-line sonarjs/no-nested-template-literals
      ...
      if (pollSummary.length > 0) lines.push(muted`Active votes:`, ...pollSummary)
