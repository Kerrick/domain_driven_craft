...
export class UninviteCommand {
  ...
  static pattern = new RegExp(`^${UninviteCommand.trigger}\\s+(\\S+)$`, "i");
  static help = { usage: `${UninviteCommand.trigger} <gamertag>`, description: "Remove a player from the server (Op)" };
  ...
  static from(message) {
    ...
    if (match) {
      return new UninviteCommand(new Gamertag(match[1]));
    }
    return null;
  }
  ...
  execute(player) {
    ...
    try {
      ...
      if (e instanceof ResidentProtected) {
        Chat.instance.whisper(player, warning`${this.#gamertag.name} is a seed player. Remove from players.yml and redeploy.`);
      } else {
        throw e;
      }
    }
