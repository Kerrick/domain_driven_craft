...
export class SettingCommand {
  ...
  #showAll(player) {
    ...
    Chat.instance.whisper(player,
      ...
      [muted`tickspeed:`, highlight`${server.baseTickSpeed}`],
      [muted`fasttickduration:`, highlight`${server.fasttickDuration}s`],
      [muted`Usage:`, command`${SettingCommand.trigger}`, arg`<name>`, arg`<value>`]
    );
  }
  ...
  #showOne(player) {
    ...
    switch (this.#name) {
      ...
      case "difficulty":
        ...
        break;
      case "tickspeed":
        chat.whisper(player, [muted`tickspeed:`, highlight`${server.baseTickSpeed}`]);
        break;
      case "fasttickduration":
        chat.whisper(player, [muted`fasttickduration:`, highlight`${server.fasttickDuration}s`]);
        break;
      default:
  ...
  #setOne(player) {
    ...
    switch (this.#name) {
      case "timeout":
        const timeout = parseInt(this.#value, 10);
        if (isNaN(timeout)) {
          chat.whisper(player, error`Invalid timeout: ${this.#value}`);
          return;
        }
        chat.pollTimeout = timeout;
      ...
      case "difficulty":
        ...
        server.difficulty = Change.permanent(difficulty);
        chat.speak([success`[Server]`, `Base difficulty set to`, highlight`${difficulty}`, `by`, highlight`${player.name}`]);
        break;
        
      case "tickspeed":
        const tickSpeed = parseInt(this.#value, 10);
        if (isNaN(tickSpeed)) {
          chat.whisper(player, error`Invalid tick speed: ${this.#value}`);
          return;
        }
        server.tickSpeed = Change.permanent(tickSpeed);
        chat.speak([success`[Server]`, `Base tick speed set to`, highlight`${tickSpeed}`, `by`, highlight`${player.name}`]);
        break;
        
      case "fasttickduration":
        const duration = parseInt(this.#value, 10);
        if (isNaN(duration)) {
          chat.whisper(player, error`Invalid duration: ${this.#value}`);
          return;
        }
        server.fasttickDuration = duration;
        chat.speak([success`[Server]`, `FastTick duration set to`, highlight`${duration}s`, `by`, highlight`${player.name}`]);
        break;
