...
export class PreferencesCommand {
  ...
  static from(message) { return message.toLowerCase().trim() === '!prefs' ? new PreferencesCommand() : null }

  execute(player) {
    const lines = [warning`=== Your Preferences ===`]
    for (const pref of player.preferences.all()) lines.push([muted`${pref.name}:`, highlight`${pref.formatted}`, muted`—`, command`${pref.command}`])
