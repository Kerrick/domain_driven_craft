...
export class Setting {
  ...
  parse(_input) { throw new Error('Subclass must implement') }
  get hasOverride() { return this.#expiresAt !== null && Date.now() < this.#expiresAt }
  ...
  apply(change) {
    ...
    if (change.isTemporary) this.#expiresAt = change.expiresAt
    else {
      this.#expiresAt = null
  ...
  force(value) { this.current = value }
