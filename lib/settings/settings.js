...
const SETTINGS = [DifficultySetting, TickSpeedSetting, FasttickDurationSetting, PollTimeoutSetting];
...
export class Settings extends Singleton {
  ...
  constructor(key) {
    super(key);
    this.#instances = SETTINGS.map(S => new S());
    this.#byName = new Map(this.#instances.map(s => [s.name.toLowerCase(), s]));
  }
