...
export class Settings extends Singleton {
  ...
  all() { return this.#instances.map((s) => s.asReadonly()) }
  get(name) { return this.#byName.get(name.toLowerCase())?.asReadonly() }
  parse(name, input) { return this.#byName.get(name.toLowerCase())?.parse(input) ?? null }

  apply(name, change) {
    const setting = this.#byName.get(name.toLowerCase())
    if (setting) setting.apply(change)
  }

  force(name, value) {
    const setting = this.#byName.get(name.toLowerCase())
    if (setting) setting.force(value)
  }
}
